...
public class Draw_Toys implements Model {
    ...
    public ArrayList<Toys> Get_Prize(ArrayList<Toys> list, ArrayDeque<Toys> list_prize) {
        if (list.isEmpty()) {
            System.out.println("Игрушки закончились, до свидания!");
        } else {
            Random r = new Random();
            ...
            for (int i = 0; i < draw_list.length; i++) {
                ...
                if (count >= Random_num) {
                    System.out.println("Вы выиграли: " + draw_list[i].getToy_name() + ", поздравляем!" + "\n");
    ...
    public ArrayDeque<Toys> Get_toy(ArrayDeque<Toys> toy_list) {
        if (toy_list.isEmpty()) {
            ...
            Date date = new Date();
            File file = new File("Shop_toys/list_toys.txt");
